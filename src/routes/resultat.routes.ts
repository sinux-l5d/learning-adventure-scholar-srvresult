...
const ajouteNouvelleTentative: RequestHandler = async (req, res, next) => {
  ...
  const idExoDBResult: string = await ResultatService.getIdExoFromExoUsrSes(idExo, idEtu, idSes);
  console.log(idExoDBResult);
  res.sendStatus(200);

  // // On convertit la tentative reçu du srveval pour un format compatible avec la bdd résultat
  // const tentativeForDB: Tentative = await ResultatService.convertAttemptForDB(tentative);

  // // On ajoute la tentative la bdd résultat
  // ResultatService.addAttemptToDB(tentativeForDB)
  //   .then(() => {
